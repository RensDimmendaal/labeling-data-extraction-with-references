...
def get_next_field(current_field):
    # TODO: replace by just inspecting the class...
    fields = ["job_title", "company", "location", "salary", "minimum_education"]
    current_index = fields.index(current_field)
    if current_index < len(fields) - 1:
        return fields[current_index + 1]
    return None
...
def generate_forms(extraction, active_field, filename):
    ...
    return Div(*field_forms, cls="field-forms")  # Return all forms wrapped in a div with a common class
...
def post(filename: str, active_field: str, fact: Fact):
    ...
    return RedirectResponse(f"/label/{filename}/{get_next_field(active_field)}", status_code=303)
